...
func main() {
	...
	builder := agentkit.NewGraphBuilder("Task Management Workflow", "Simple workflow to handle task creation and assignment")

	// Define agents for the workflow

	// Task Creator agent - Creates and describes tasks
	taskCreatorAgent := &agentkit.Agent{
		Name: "TaskCreator",
...
Always be clear and concise in your responses.`,
	...
	taskAssignerAgent := &agentkit.Agent{
		Name: "TaskAssigner",
...
Be efficient and fair in assignments.`,
	...
	taskTrackerAgent := &agentkit.Agent{
		Name: "TaskTracker",
...
Be thorough and organized in your tracking.`,
	...
	builder.WithNode("creator_tracker", "Creator Tracker", func(ctx context.Context, state agentkit.GraphState) (agentkit.GraphState, error) {
		newState := state.Clone()
		newState["last_node"] = "creator"
		return newState, nil
	})

	builder.WithNode("assigner_tracker", "Assigner Tracker", func(ctx context.Context, state agentkit.GraphState) (agentkit.GraphState, error) {
		newState := state.Clone()
		newState["last_node"] = "assigner"
		return newState, nil
	})

	builder.WithNode("tracker_tracker", "Tracker Tracker", func(ctx context.Context, state agentkit.GraphState) (agentkit.GraphState, error) {
		newState := state.Clone()
	...
	builder.WithNode("router", "Task Router", func(ctx context.Context, state agentkit.GraphState) (agentkit.GraphState, error) {
	...
	builder.WithNode("intake", "Task Intake", func(ctx context.Context, state agentkit.GraphState) (agentkit.GraphState, error) {
	...
	// Create exit node
	builder.WithNode("exit", "Task Completion", func(ctx context.Context, state agentkit.GraphState) (agentkit.GraphState, error) {
		newState := state.Clone()
	...
	if messagesRaw, ok := finalState[agentkit.MessageKey]; ok {
