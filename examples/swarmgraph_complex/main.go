...
func FetchPatientRecord(args map[string]interface{}, contextVariables map[string]interface{}) agentkit.Result {
...
func ScheduleAppointment(args map[string]interface{}, contextVariables map[string]interface{}) agentkit.Result {
...
func OrderLabTest(args map[string]interface{}, contextVariables map[string]interface{}) agentkit.Result {
	patientID, ok1 := args["patient_id"].(string)
...
func PrescribeMedication(args map[string]interface{}, contextVariables map[string]interface{}) agentkit.Result {
	patientID, ok1 := args["patient_id"].(string)
...
func main() {
	...
	builder := agentkit.NewGraphBuilder("Medical Clinic Workflow", "Workflow for patient processing in a medical clinic")

	// 1. Receptionist Agent - Handles patient intake and scheduling
	receptionistAgent := &agentkit.Agent{
		Name: "Receptionist",
...
Be caring, attentive, and thorough in your assessments.`,
		...
		Functions: []agentkit.AgentFunction{
			...
			},
			{
				...
				Function: func(args map[string]interface{}, contextVars map[string]interface{}) agentkit.Result {
...
Be precise, methodical, and attentive to detail.`,
	...
	pharmacistAgent := &agentkit.Agent{
		Name: "Pharmacist",
...
Be meticulous, knowledgeable, and patient-focused.`,
	...
	builder.WithNode("reception_tracker", "Reception Tracker", func(ctx context.Context, state agentkit.GraphState) (agentkit.GraphState, error) {
		newState := state.Clone()
		newState["last_node"] = "reception"
		return newState, nil
	})

	builder.WithNode("nurse_tracker", "Nurse Tracker", func(ctx context.Context, state agentkit.GraphState) (agentkit.GraphState, error) {
		newState := state.Clone()
		newState["last_node"] = "nurse"
		return newState, nil
	})

	builder.WithNode("doctor_tracker", "Doctor Tracker", func(ctx context.Context, state agentkit.GraphState) (agentkit.GraphState, error) {
		newState := state.Clone()
		newState["last_node"] = "doctor"
		return newState, nil
	})

	builder.WithNode("lab_tracker", "Lab Tracker", func(ctx context.Context, state agentkit.GraphState) (agentkit.GraphState, error) {
		newState := state.Clone()
	...
	builder.WithNode("router", "Patient Router", func(ctx context.Context, state agentkit.GraphState) (agentkit.GraphState, error) {
	...
	routerCondition := func(state agentkit.GraphState) (agentkit.NodeID, error) {
		...
		// Get messages
		messagesRaw, ok := state[agentkit.MessageKey]
		...
		if len(messages) > 15 || visits["reception"] > 5 {
			...
			if _, hasPatient := state["patient"]; !hasPatient {
				...
				if messagesArr, ok := messagesRaw.([]llm.Message); ok {
					...
					state[agentkit.MessageKey] = messagesArr
				}
			}
	...
	builder.WithNode("intake", "Patient Intake", func(ctx context.Context, state agentkit.GraphState) (agentkit.GraphState, error) {
		...
		newState[agentkit.MessageKey] = initialConversation
	...
	builder.WithNode("exit", "Checkout Process", func(ctx context.Context, state agentkit.GraphState) (agentkit.GraphState, error) {
		// Process checkout, billing, etc.
		newState := state.Clone()

		// Get messages
		messagesRaw, exists := state[agentkit.MessageKey]
		...
		newState[agentkit.MessageKey] = messages
