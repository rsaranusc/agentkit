		...
		Functions: []agentkit.AgentFunction{
			{
				...
				Function: func(args map[string]interface{}, contextVariables map[string]interface{}) agentkit.Result {
					op := cast.ToString(args["operation"])
					x := cast.ToFloat64(args["x"])
					y := cast.ToFloat64(args["y"])
					...
					case "divide":
						if y == 0 {
							return agentkit.Result{Success: false, Data: "Error: division by zero"}
						}
						result = x / y
					default:
						return agentkit.Result{Success: false, Data: fmt.Sprintf("Error: unknown operation: %s", op)}
					}

					if err != nil {
						return agentkit.Result{Success: false, Data: fmt.Sprintf("Error: %v", err)}
					}

					return agentkit.Result{Success: true, Data: fmt.Sprintf("%.2f", result)}
				},
			},
		},
	}
